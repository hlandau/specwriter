...
;; Normative section
(define (§ title . rest)
  (sxml-with-attribute (apply sec (cons title rest)) 'kind 'normative))
;; Informative section
(define (§i title . rest)
  (sxml-with-attribute (apply sec (cons title rest)) 'kind 'informative))
;; Annex
(define (§a title . rest)
  (sxml-with-attribute (apply sec (cons title rest)) 'kind 'annex))
...
(export doc docctl doctitle docbody sec § §i §a §x §xi p ul ol li bq procn math mathbl mathbb em kw pre code
        shall shall-not should should-not may may-not can cannot must note figure table span dict dice hexcode
        tabular tr td th
        number-clauses number-figures number-things)
|#
